...
public:
	...
	MarchFrpLauncher(string exePath, string Arch, string SubPlatform = "Linux") {
	...
	void welcome() {
		system("mkdir ~/.MarchFrpLauncher");
		printf("  ____   _____ ______\n / __ \\ / ____|  ____|\n| |  | | |  __| |__ _ __ _ __\n| |  | | | |_ |  __| '__| '_ \\\n| |__| | |__| | |  | |  | |_) |\n \\____/ \\_____|_|  |_|  | .__/\n                        | |\n                        |_|\n\n");
		printf("Welcome! MarchFrpLauncher.%s (MarchFrpLauncher.Unix Core Version %s %s)\n", SubPlatform.c_str(), Version, Arch.c_str());
		printf("\n");
		printf("  * Website:   https://marchfrplauncher.cn \n");
		printf("  * GitHub:    https://github.com/oldgodshen/marchfrplauncher \n");
	...
	void lsfrps() {
		system(("curl --location --request POST 'https://api填' --form 'action=getnodes' --form 'token=" + token + "'").c_str());
		printf("\n");
	}
	...
	void startfrpc(string nodeid, string actoken) {
		...
		string iniPath = "~/.MarchFrpLauncher/frpc.ini";
		string curlsh = "curl --location --request POST 'https://api填' --form 'action=getconf' --form 'token=" + token + "' --form 'node=" + nodeid + "' -o " + iniPath;
		system(curlsh.c_str());
	...
	void printHelp() {
		printf("-----MarchFrpLauncher.Linux helper-----\n");
		printf("exit               Quit.\n");
		...
		printf("token [token]      Set your MarchFrpLauncher access token. | token: your MarchFrpLauncher access token\n");
	}

	int shell(string path) {
		while (true) {
			printf("MarchFrpLauncher> ");
			string script;
			getline(cin, script);
			string cmd;
			{
