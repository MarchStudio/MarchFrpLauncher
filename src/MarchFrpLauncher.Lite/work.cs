...
namespace MarchFrpLauncher.Lite
{
    ...
    public class work
    {
        ...
        public work()
        {
            ...
            try
            {
                FileSystem.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\MarchFrpLauncher");
            }
            catch { }
            while (true)
            {
        ...
        void printWelcome()
        {
            ...
            Console.Write("Welcome! MarchFrpLauncher.Lite (Version {0}, Windows Generic, .NET Framework {1})\n", Heading.Version, Heading.dotnetfxVersion);
            Console.Write("\n");
            Console.Write("  * Website:   https://marchfrplauncher.cn \n");
            Console.Write("  * GitHub:    https://github.com/oldgodshen/marchfrplauncher \n");
        ...
        {
            Console.Write("-----MarchFrpLauncher.Lite helper-----\n");
            ...
            Console.Write("token [token]      Set your MarchFrpLauncher access token. | token: your MarchFrpLauncher access token\n");
        }
        ...
        void startfrpc(string nodeid, string actoken)
        {
            try
            {
                ...
                string iniPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\MarchFrpLauncher" + "\\frpc.ini";
                string iniurl = "https://api填/?action=getconf&token=" + actoken + "&node=" + nodeid;
        ...
        void lsfrps()
        {
            try
            {
                string frpslist = UI.Net.Get("https://api填/?action=getnodes&token=" + token);
        ...
        void shell()
        {
            Console.Write("MarchFrpLauncher> ");
